...
2. **Tetration core** - we iterate
   \[
   z_{n+1}=\exp\!\left(C(z_n)\log(c_{\mathrm{eff}}(z_n))\right)
   \]
   where \(c_{\mathrm{eff}}(z_n)=c\cdot e^{i\phi_n}\), and the phase kick \(\phi_n\) depends on \(\mathrm{Re}(C(z_n))\), for example
   \[
   \phi_n=\alpha\tanh\!\left(\frac{\mathrm{Re}(C(z_n))}{s}\right)
   \]
   with scale parameter \(s>0\).
