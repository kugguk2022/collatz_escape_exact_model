...
2. **Tetration core** - we iterate
   ...
   \]
   where \(c_{\text{eff}}(z_n) = c\cdot e^{i\phi_n}\) and the phase kick \(\phi_n\) depends on \(\mathrm{Re}(C(z_n))\) (e.g., \(\phi_n = \alpha\tanh(\mathrm{Re}(C(z_n))/s)\), with scale parameter \(s\)).
